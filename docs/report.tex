...
\screenshot{screenshots/visualvm-mbeans-chart-points.png}{График изменения показаний MBean в VisualVM}
...
Для локализации проблемы производительности использовалась программа
\texttt{HttpUnit} под нагрузкой. Анализ проводился с помощью Java Mission
Control (JMC) и записей Java Flight Recorder (JFR).

\subsection{Выявленная проблема}

\screenshot{screenshots/jmc-heap-usage.jpg}{График изменения Heap Usage}

При анализе JFR-записи обнаружено, что после каждого цикла сборки мусора
нижняя граница используемой памяти не возвращается к исходному уровню, а
постепенно растёт. В таблице классов с наибольшим приростом живущих объектов
(`Live Size Increase`) лидировали классы \texttt{byte[]} и
\texttt{java.util.TreeMap\$Entry}. В событии \texttt{Old Object Sample}
объекты этих классов имели большой возраст и сохранялись через GC Root,
что указывало на удерживание ссылок и невозможность их удаления сборщиком
мусора.

\subsection{Алгоритм локализации в JMC}

\begin{enumerate}
    \item Запустить приложение с включённым JFR и опцией \texttt{Old Object Sample}.
    \item Открыть запись в JMC, перейти в раздел \texttt{Java Application → Memory}.
    \item На графике \texttt{Used Heap} проверить, что после каждого GC нижняя граница памяти возрастает (признак утечки).
    \item В таблице отсортировать классы по показателю \texttt{Live Size Increase} и выделить те, у которых рост максимален.
    \item В \texttt{Event Browser} выбрать \texttt{Old Object Sample} для подозрительных классов и изучить:
    \begin{itemize}
        \item \texttt{Object Age} — время жизни объектов;
        \item \texttt{Last Known Heap Usage} — занимаемый объём;
        \item \texttt{GC Root} — причину, по которой объекты остаются достижимыми.
        \item \texttt{Stack Trace} — методы которые создают эти объекты.
    \end{itemize}
    \item Сделать вывод: утечка подтверждается, если после GC память не уменьшается, класс стабильно лидирует
        по приросту, а его объекты долго живут и создаются через вызовы нашего кода.
\end{enumerate}

\screenshot{screenshots/jmc-target.jpg}{Таблица Old Object Sample}

В результате для \texttt{HttpUnit} подозрительными оказались \texttt{byte[]} и \texttt{TreeMap\$Entry},
но \texttt{TreeMap} исходит из внутренних классов JMV когда \texttt{byte[]} лежит в строках
создаваемых в нашем методе.
...
Также была локализована и устранена проблема производительности в \texttt{HttpUnit}: статический список \texttt{JavaScript.\_errorMessages} удерживал старые строки с сообщениями об ошибках. После исправления количество сохраняемых сообщений ограничено.
\end{document}
